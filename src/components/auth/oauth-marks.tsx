...
export function FacebookMark(props: ComponentProps<"svg">) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden {...props}>
      {/* Drawn in `currentColor` so the caller picks: a knockout on a dark surface, or
          the brand blue on a light one. The sign-in row is glass over a dark page, so it
          inherits the button’s own near-white — which is what Meta specifies for a dark
          button, and the only version with usable contrast there. */}
      <path
        fill="currentColor"
